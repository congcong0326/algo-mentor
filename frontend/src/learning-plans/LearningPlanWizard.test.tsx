...
function goToTopicStep(goal = '准备面试') {
...
describe('LearningPlanWizard', () => {
  ...
  it('preserves multi-word comma-separated topics', () => {
    const onSubmit = vi.fn();

    render(<LearningPlanWizard loading={false} onCancel={vi.fn()} onSubmit={onSubmit} />);

    goToTopicStep('补齐图论和动态规划');

    fireEvent.change(screen.getByRole('textbox', { name: '添加主题' }), {
      target: { value: 'Array, Hash Table，Dynamic Programming' },
    });
    fireEvent.keyDown(screen.getByRole('textbox', { name: '添加主题' }), { key: 'Enter' });
    fireEvent.click(screen.getByRole('button', { name: '下一步' }));
    fireEvent.click(screen.getByRole('button', { name: '生成草案' }));
    ...
    expect(onSubmit).toHaveBeenCalledWith(expect.objectContaining({
      topicPreferences: ['Array', 'Hash Table', 'Dynamic Programming'],
    }));
  });

  it('splits pure whitespace topics and exposes removable tag buttons', () => {
    render(<LearningPlanWizard loading={false} onCancel={vi.fn()} onSubmit={vi.fn()} />);

    goToTopicStep();

    fireEvent.change(screen.getByRole('textbox', { name: '添加主题' }), {
      target: { value: 'Graph Tree DP' },
    });
    fireEvent.keyDown(screen.getByRole('textbox', { name: '添加主题' }), { key: 'Enter' });

    expect(screen.getByRole('button', { name: '移除主题 Graph' })).toHaveTextContent('Graph');
    expect(screen.getByRole('button', { name: '移除主题 Tree' })).toHaveTextContent('Tree');
    expect(screen.getByRole('button', { name: '移除主题 DP' })).toHaveTextContent('DP');

    fireEvent.click(screen.getByRole('button', { name: '移除主题 Graph' }));

    expect(screen.queryByRole('button', { name: '移除主题 Graph' })).not.toBeInTheDocument();
  ...
  it('prevents moving past invalid numeric values', () => {
    ...
    expect(screen.getByRole('button', { name: '下一步' })).toBeDisabled();
    expect(screen.getByText('周期和每周小时数必须是正整数。')).toBeInTheDocument();
  });

  it('prevents moving past decimal numeric values', () => {
    render(<LearningPlanWizard loading={false} onCancel={vi.fn()} onSubmit={vi.fn()} />);
