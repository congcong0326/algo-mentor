...
export const localeResources: Record<SupportedLocale, LocaleResources> = {
  'zh-CN': {
    ...
    learningPlans: {
      ...
      practiceLeetCodeGuidance: '下方题目内容由 AI 自动生成，最终以 LeetCode 为准；点击右侧图标跳转到 LeetCode。代码测试推荐在 LeetCode 上完成，成功或失败都建议把提交结果、报错或反馈粘贴到对话框，让 AI 分析并沉淀用户画像，后续更方便推荐题目。',
  ...
  'en-US': {
    ...
    learningPlans: {
      ...
      practiceLeetCodeGuidance: 'The problem content below is generated by AI, and LeetCode is the source of truth. Use the icon on the right to open LeetCode. Run code tests on LeetCode, then paste accepted or failed submissions, errors, or feedback into the chat so AI can analyze them and build your learning profile for better future recommendations.',
