...
public class PracticeChatPromptSectionProvider implements PromptSectionProvider {
  ...
  private static final String COACHING_POLICY = """
      ...
      只要当前用户消息看起来像是在粘贴当前题目的完整 LeetCode 解法，应优先调用正式 Review 工具，不要先绕过工具输出完整 Review。
      如果工具未被调用，或用户拒绝确认或确认超时，可以继续普通点评代码，但只能给非正式建议。
      如果只是片段、报错、伪代码、局部 bug、语法问题、复杂度讨论或概念问题，请正常答疑，并引导用户粘贴完整 LeetCode Solution 代码生成正式 Review。
  ...
  private static final String CODE_REVIEW_TOOL_BOUNDARY = """
      ...
      2. 即使用户没有明确要求正式 Review，也应调用 %s，让用户通过确认弹窗决定是否生成正式记录。
      3. %s 会记录一次正式代码提交，委托 Review 流程抽取代码、分析、打分并保存 Review 记录；系统会在执行前请求用户确认，工具不能绕过确认。
      4. 如果不确定是否完整但确实像题解提交，偏积极触发；明显片段、伪代码、报错日志、局部 bug、语法问题、复杂度讨论和概念问题不要调用工具，应按普通答疑处理。
      5. 如果用户拒绝确认或确认超时，可以继续普通点评代码，但必须说明没有生成正式 Review 记录，不要给出正式分数，不要声称已完成正式 Review，也不要声称已生成 Review 记录，不要声称完成状态已更新。
