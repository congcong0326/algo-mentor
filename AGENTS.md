...
## 协作语义
- 当用户说“结合当前项目”“基于当前项目”“用当前项目做测试/demo”等类似表述时，默认“当前项目”固定指 `algo-mentor` 仓库，即用户语境中的 `/code/algo-mentor` 项目，而不是其他临时目录、示例仓库或外部项目。
- 一般需要测试、验证、demo 或提炼项目经验时，优先围绕 `/code/algo-mentor` 的实际代码、结构和业务背景展开；除非用户明确指定其他项目，不要切换到无关项目。
