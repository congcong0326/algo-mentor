...
`algo-mentor` 上线前需要管理员能够判断服务是否稳定运行，尤其是 HTTP API、SSE 流式连接、Agent 执行和学习计划生成等高频路径是否出现异常。

本需求不建设自研监控后台。通用监控、日志检索、告警通知和 Dashboard 展示交给成熟观测系统承担，项目只补外部系统无法自动理解的业务语义信号。

## 目标

- 接入 Prometheus、Grafana、Loki、Promtail，形成本地和上线前可复用的观测栈。
...
Prometheus/Grafana/Loki 地址通过 Docker Compose 内部网络连接，不写入业务配置。生产环境是否启用外部 Prometheus 抓取，由部署平台配置。
...
- Docker Compose 观测栈适合本地和小规模部署参考，生产环境可能需要托管 Prometheus/Grafana/Loki 或云厂商方案。
- `agentRunId`、`requestId` 等不应作为指标标签，也不应作为 Loki label；需要查单次问题时通过日志全文字段查询。

## 验收标准

- 使用 profile 能启动 Prometheus、Grafana、Loki、Promtail。
