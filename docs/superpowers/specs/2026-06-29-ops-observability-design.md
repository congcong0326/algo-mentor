...
- `prometheus`
  ...
  - Prometheus 自身使用默认 `9090` 端口；被采集的 mentor-api 程序使用 `8080`。
- `grafana`
  - 自动配置 Prometheus 和 Loki datasource。
  - Grafana 访问 Prometheus 使用 Docker 网络内地址 `http://prometheus:9090`。
  - 自动加载 dashboard JSON。
- `loki`
  - 接收 Promtail 推送的容器日志。
- `promtail`
...
- mentor-api 在 `8080` 暴露 `/actuator/prometheus`，Prometheus 在 `9090` 提供自身 Web UI 和 readiness。
