# Agent 工具权限与 Forced Tool Calling 设计
...
因此第一阶段不追求“模型一定调用 Review 工具”，也不做后端代码意图识别或 forced tool calling。第一阶段先把最小闭环做出来：
...
forced tool calling 仍是后续必要能力，适合显式“提交 Review”入口，但不放在第一阶段。
...
阶段零完成后，用户粘贴完整代码只会进入普通聊天 run，不会自动生成正式 Review 记录。正式 Review 的写库入口后续只来自 Review 工具或显式 Review run。
...
用户点击“提交 Review”
-> 后端创建明确 Review run
-> tool_choice = SPECIFIC(submit_practice_code_review)
-> 仍可经过权限机制，或将显式点击视为本次允许
```

阶段二解决的是确定性调用问题，阶段一解决的是工具执行安全问题。
...
- 为后续 per-run forced tool calling 预留扩展点，但第一阶段不实现。

## 非目标

- 第一阶段不做 forced tool calling。
...
第一阶段不做 forced tool calling，因此工具描述要尽量清晰，让模型在适合时自主调用。

`submit_practice_code_review` 描述建议：

```text
当用户明确请求正式代码 Review，或在题目练习中提交完整解法希望判断是否通过时，调用此工具。
不要在用户只是询问概念、语法、局部 bug、异常日志或普通思路时调用。
调用前系统会请求用户确认。
```
...
```text
如果需要执行正式代码 Review，应调用 submit_practice_code_review 工具。
该工具是高权限工具，系统会在执行前请求用户确认。
如果用户拒绝工具执行，请继续用普通聊天方式帮助用户。
```
...
## 后续 Forced Tool Calling 扩展

阶段二再引入 per-run tool policy。
...
- 第二阶段通过显式入口 + forced tool calling 解决确定性。
...
### 阶段二：显式 Review 入口与 Forced Tool Calling

- 增加 `AgentToolExecutionOptions`。
- `AgentExecutionOptions` 携带 per-run tool options。
- `AgentLoopRunner` 支持 per-run effective tools 和 effective tool choice。
- `AgentConversationCommand` 传播 execution options。
- 显式 Review 入口使用 `SPECIFIC(submit_practice_code_review)`。
- 修复 `withLockToken()` 保留 execution options。
